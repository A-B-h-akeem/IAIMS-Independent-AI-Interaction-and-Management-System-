...
OPENROUTER_API_KEY = "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"  # Get from https://openrouter.ai/keys
...
def calculate_expiry(priority="medium"):
    now = datetime.now()
    ...
    if "high" in p:
        ...
        label = "HIGH (1 Year)"
        print(f"DEBUG: Logic matched HIGH.")
    elif "low" in p:
        expiry = now + timedelta(days=3)
        label = "LOW (3 Days)"
        print(f"DEBUG: Logic matched LOW.")
    else:
        expiry = now + timedelta(days=30)
        label = "MEDIUM (30 Days)"
        print(f"DEBUG: Falling back to MEDIUM. Received: '{p}'")
    
    # RETURN TWO VALUES
    return expiry.strftime("%Y-%m-%d %H:%M:%S"), label

def save_memory(fact: str, tags: list = None, priority="medium"):
    """
    Saves a fact to Membrain with dynamic TTL based on priority.
    """
    if tags is None:
        tags = []
    
    # CRITICAL: Calculate the expiry date NOW to get the current timestamp
    expiry_time, priority_label = calculate_expiry(priority)
    
    # Embed metadata into tags for future cleanup/filtering
    meta_tags = tags + [
        "clawmind", 
        "hackathon", 
        "ishan", 
    ...
    print(f"\n💾 [Memory Engine: Saving with {priority_label} priority]")
    print(f"⏳ [Auto-delete scheduled: {expiry_time}]")
    ...
    try:
        ...
        # Poll Membrain for processing completion
        for _ in range(25):
            time.sleep(1)
            poll_res = requests.get(
                f"{MEMBRAIN_BASE_URL}/memories/jobs/{job_id}", 
                headers=membrain_headers, 
                timeout=10
            )
            ...
            if status_data.get("status") == "completed":
                print(f"✅ SUCCESS: Saved to graph. Expiry: {expiry_time}")
                return f"Memory saved. Priority: {priority_label}. Expiry: {expiry_time}"
            
            elif status_data.get("status") == "failed":
                return f"FAILED: {status_data.get('error', 'Unknown database error')}"

        return "Save request timed out, but likely processing in background."
    
    except Exception as e:
        print(f"⚠️ Save tool error: {e}")
        return f"Save tool failed: {str(e)}"
# ===================== FILE SYSTEM TOOL =====================
...
You now support Managed Context with Priority-based Auto-Deletion.

Rules:
- Always search memory first for context.
- When user shares info, use the "remember" action and assign a PRIORITY.
- PRIORITY LEVELS:
  - HIGH: Core project data, personal identity (Kept for 1 year).
  - MEDIUM: General tasks, meeting notes (Kept for 30 days).
  - LOW: Trivial info, coffee chats, temporary reminders (Kept for 3 days).
- For WhatsApp and Facebook actions, ALWAYS ask for permission first.
...
{"action": "search", "query": "..."}
{"action": "remember", "fact": "...", "priority": "high|medium|low", "tags": ["tag1"]}
    ...
    while True:
        ...
        for _ in range(10):   # Safety limit on ReAct steps
            try:
                ...
                # FIND THIS IN YOUR run_agent() LOOP:
                elif action == "remember":
                    # 1. Get priority from the AI's JSON (the prompt tells it to use "priority")
                    priority_level = decision.get("priority", "medium").lower()
                    
                    # 2. Hard override if the user explicitly typed it
                    if "high" in user_input.lower():
                        priority_level = "high"
                    elif "low" in user_input.lower():
                        priority_level = "low"

                    # 3. Call save_memory with the determined priority
                    result = save_memory(
